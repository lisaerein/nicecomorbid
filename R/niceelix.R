...
niceelix <- function(df
      ...
      for (i in 1:length(evars)){
            ...
            if (score == "readmission") emat[,i] <- df[,evars[i]]*eval(as.symbol(paste("erw",evars[i], sep="_")))
            if (score == "mortality"  ) emat[,i] <- df[,evars[i]]*eval(as.symbol(paste("emw",evars[i], sep="_")))
